...
namespace Tracker.Core.Repositories
{
    public class ProjectSitesRepository : IProjectSitesRepository
    {
        ...
        public IEnumerable<siteDto> GetAllSitesByProjectId(int ProjectId)
        {
            List<siteDto> lstSites = new List<siteDto>();
            if (ProjectId == 0)
            {
                throw new NotExistException("Not Exist Exception");
            }
            else
            {
                var ProjectSites = _context.ProjectSites.Where(p => p.ProjectId == ProjectId).Select(projectsite => new ProjectSitesDTO
                {
                    ...
                    SiteId = projectsite.SiteId,
                    SiteName = projectsite.Sites.Sitename,
                    phone=projectsite.Sites.Phone,
                    address=projectsite.Sites.Address,
                    govid=projectsite.Sites.GovernorateId,
                    cityid=projectsite.Sites.CityId,
                    cityName=projectsite.Sites.City.cityName,
                    GovName=projectsite.Sites.Governorate.governorateName

              
                }).ToList();
                foreach (var item in ProjectSites)
                {
                    siteDto sites = new siteDto();
                    sites.Id = item.SiteId;
                    sites.Sitename = item.SiteName;
                    sites.Address = item.address;
                    sites.Phone = item.phone;
                    sites.CityId = item.cityid;
                    sites.GovernorateId = item.govid;
                    sites.CityName = item.cityName;
                    sites.GovernorateName = item.GovName;
                    lstSites.Add(sites);
                }
                return lstSites;
            }
